...
import { SafeAreaView } from "react-native-safe-area-context";
import api from "../../src/api/api"; // Assuming api is correctly defined in your project
...
const UploadDocumentScreen: React.FC = () => {
  ...
  const handleVerify = async () => {
    ...
    if (!refNo.trim()) {
      setErrors((prev) => ({
        ...prev,
        refNo: `Please enter a ${
          mode === "CUSTOMER_ID" ? "Customer ID" : "Account Number"
        }`,
      }));
      return;
    }
    ...
    try {
      ...
      // 🔹 Handle response
      if (data.RC === "0") {
        // Log updated token (assuming it's handled elsewhere)
        const updatedToken = await AsyncStorage.getItem("TokenNo");
        console.log("🔐 Token after verify:", updatedToken);
  ...
  return (
    <View style={styles.container}>
      ...
      <KeyboardAvoidingView
        ...
        behavior={Platform.OS === "ios" ? "padding" : "height"}
        // Optional: Set a small offset for iOS if the button is slightly covered
        keyboardVerticalOffset={Platform.OS === "ios" ? 0 : -40}
      >
        {/* This inner view holds ALL content and is the only element that the KAV will move */}
...
const styles = StyleSheet.create({
  ...
  innerContent: {
    flex: 1,
    // Add horizontal padding here so all elements (including the button) are centered
    paddingHorizontal: "7.5%",
    // Justify content to the center to lift it naturally when the keyboard appears
    justifyContent: "center",
    alignItems: "center",
    paddingBottom: 10, // Ensure there is space at the bottom when keyboard is hidden
  },
  header: {
    backgroundColor: "#F9B300",
    width: "100%",
  ...
  scanText: {
    ...
    fontWeight: "bold",
    color: "#000",
    marginTop: 10, // Removed large marginTop
    marginBottom: 50, // Added vertical spacing
  },
  radioGroup: {
    flexDirection: "row",
    justifyContent: "center",
  ...
  button: {
    ...
    borderRadius: 8,
    alignItems: "center",
    marginTop: 20, // Reduced spacing between error/input and button
  },
  buttonDisabled: {
    backgroundColor: "#9E9E9E",
    opacity: 0.6,
